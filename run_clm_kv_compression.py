...
def main():
    ...
    fileHandler = logging.FileHandler(filename=os.path.join(args.output_dir, "train.log"))
    ...
    if args.dataset_name is not None:
        ...
        if '103' in args.dataset_config_name:
            raw_datasets["train"] = load_dataset(
                args.dataset_name,
                args.dataset_config_name,
                split=f"train[70%:]",
    ...
    # if any(n in args.model_name_or_path for n in ['xl', 'neo']):
    ...
    # add compression tokens
    if args.compress:
        num_tokens_added = tokenizer.add_tokens([CL_TOKEN, CR_TOKEN], special_tokens=False)
        cl_token_id, cr_token_id = tokenizer.convert_tokens_to_ids([CL_TOKEN, CR_TOKEN])
        logger.info(f"Added {num_tokens_added} tokens")
        logger.info(f"CL_TOKEN: {cl_token_id}, CR_TOKEN: {cr_token_id}")

    if args.model_name_or_path:
        model_cls = get_custom_model_cls(args.model_name_or_path)
    ...
    if len(tokenizer) > embedding_size:
        ...
        logger.info(f"Token embedding resized to {len(tokenizer)}")
    ...
    if args.peft == 'lora':
        ...
        if any(n in args.model_name_or_path for n in ['opt', 'gptj']):
            config = LoraConfig(
                r=lora_r,
        ...
        elif any(n in args.model_name_or_path for n in ['codegen']):
            config = LoraConfig(
                r=lora_r,
                lora_alpha=lora_alpha,
                lora_dropout=lora_dropout,
    ...
    def tokenize_function(examples):
        ...
        MAX_BOUNDED_LEN = args.max_span_length
        avg_length = (2+MAX_BOUNDED_LEN) / 2
        MAX_BOUNDED_RATIO = args.bound_ratio # ratio before taking added <CR> into account
    ...
    def group_texts(examples):
        ...
        if not args.compress:
            return result
        cnt = 0
        # adjust the labels for causal language modeling
        for i in range(len(result['input_ids'])):
            for j in range(len(result['input_ids'][i])):
                if result['input_ids'][i][j] in [cl_token_id, cr_token_id]:
                    cnt += 1
                if j+1<args.block_size:
                    if result['input_ids'][i][j] in [cl_token_id, cr_token_id]:
                        result['labels'][i][j+1] = -100
                    elif result['labels'][i][j+1] in [cl_token_id, cr_token_id]:
                        if j+2<args.block_size:
                            result['labels'][i][j+1] = result['labels'][i][j+2]
                        else:
                            result['labels'][i][j+1] = -100
        logger.info(f"Train with {cnt//2} bounded text spans")
        new_attn_mask = [[[0]*len(result['attention_mask'][0]) for _ in range(len(result['attention_mask'][0]))] for _ in range(len(result['attention_mask']))]
        ...
        for i in range(len(result['attention_mask'])):
            for j in range(len(result['attention_mask'][i])):
    ...
    for index in random.sample(range(len(train_dataset)), 3):
        logger.info(f"Sample {index} of the training set: {train_dataset[index]['input_ids']}.")
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                if any(n in args.model_name_or_path for n in ['Red']):
                    ...
                    unwrapped_model = accelerator.unwrap_model(model).base_model
                    if unwrapped_model.get_input_embeddings().weight.grad is not None:
                        unwrapped_model.get_input_embeddings().weight.grad[:-2, :] = 0
                optimizer.step()
                lr_scheduler.step()
                optimizer.zero_grad()
            ...
            if accelerator.sync_gradients:
                ...
                completed_steps += 1
                writer.add_scalar("lm_loss", loss.item(), global_step=completed_steps)
            ...
            interval = 300 if not '103' in args.output_dir else 900
            if completed_steps % interval == 0:
                logger.info(f"Current training steps {completed_steps}")
                # check_CausalLM(logger, args, model, tokenizer, torch.device("cuda:0"))
                # check_icl(logger, args, model, tokenizer, torch.device("cuda:0"))
